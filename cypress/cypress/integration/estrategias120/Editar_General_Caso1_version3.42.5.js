...
describe('Editar General', () => {
	...
	it('Invitar people en pagina inicial', () => {
		funciones.login('g_publica_post_schedule')
		 ...
		 cy.get('input[class^="ember-text-field gh-input ember-view"]').first().focus().clear().type(dato.titulo)
		 cy.get('input[class^="ember-text-field gh-input ember-view"]').last().focus().clear().type(dato.name)
