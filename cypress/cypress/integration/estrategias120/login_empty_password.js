...
context('Login & log out: empty login password', () => {
    ...
    it("Invalid login", () => {
        ...
        cy.get('form').within(($form) => {
            cy.get('input[name="identification"]').type(`${faker.internet.email()}`, {force: true})
            cy.get('input[name="password"]').type(jsonData[rndInt].value, {force: true})
            cy.root().submit();
