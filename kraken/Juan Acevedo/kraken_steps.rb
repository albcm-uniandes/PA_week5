...
Then(
  /^I enter "([^\"]*)" into input field having tag "([^\"]*)"$/
) do |text, selector|
  @driver.find_element(:tag_name, selector).send_keys(text)
  sleep 2
end

Then(
  /^I enter "([^\"]*)" into input field having article "([^\"]*)"$/
) do |text, selector|
  @driver.find_element(:article, selector).send_keys(text)
  sleep 2
end

Then(/^I click on element having class "(.*?)"$/) do |selector|
